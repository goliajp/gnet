...
All notable changes to `mesh-noise` are documented here. The format follows
...
- Extracted from `meshcli` into a standalone zero-dependency stone: the
  `Noise_IK_25519_ChaChaPoly_BLAKE2s` handshake (the pattern WireGuard uses),
  built bottom-up on the in-house `mesh-crypto` primitives — `cipher_state`
...
- Hybrid (Noise_IK + ML-KEM-768) handshake bench + perf_gate (40 ms budget) +
  BUDGETS row. The hybrid handshake is the one `meshcli` actually runs;

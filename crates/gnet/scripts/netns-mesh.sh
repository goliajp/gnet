...
# namespaces on a shared underlay bridge, each running `meshcli up` with the
...
#
#   sudo bash crates/meshcli/scripts/netns-mesh.sh
set -u
...
# (e.g. docker hosts). Targeted to br-mesh only; removed on cleanup.
...
echo "=== full-mesh overlay ping (sessions established) ==="
rc=0

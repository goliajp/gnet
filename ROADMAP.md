...
| **v0.24** | Track E presentation (CI enable, **always-latest toolchain policy**, CONTRIBUTING, deploy docs, README, codebase reflow against current rustfmt) | quality gate green | ✓ shipped 2026-06-03 — toolchain policy is "always latest stable" (rust-toolchain channel = "stable"; CI uses `dtolnay/rust-toolchain@stable`). CI is registered + green on develop (run #4 `26854620641`: fmt + clippy + workspace tests + rustdoc + perf_gate + examples + publish dry-run all pass). The initial registration needed a `workflow_dispatch:` nudge — GitHub's auto-registration of `ci.yml` had been silently failing since the 2026-05-28 split. Note: `on: push` events are not currently firing CI automatically (0 push-triggered runs); only `workflow_dispatch` works. Suspected repo setting around first-time-contributor approval — investigate before relying on push-fire. |
